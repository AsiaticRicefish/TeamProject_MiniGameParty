...
namespace LDH_Lobby
{
    public class LobbyNavigationController : MonoBehaviour
    {
        ...
        [Header("UI")] [SerializeField] private MenuEntry[] menuEntries;
        ...
        private async UniTaskVoid SwitchToAsync(string id)
        {
            ...
            if (!_currentFocusId.IsNullOrEmpty() && _currentFocusId.Equals(id))
                return; //이미 focus인 걸 또 focus 하는 경우 return

            _isSwitching = true;

            InputLockController.Instance?.Lock(); // 전환하는 동안 입력 막기
